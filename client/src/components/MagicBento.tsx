...
const defaultCards: BentoCardData[] = [
  { color: '#0d0d0f', title: 'E-Commerce Platform', description: 'Full-stack marketplace with React, Node.js, and PostgreSQL. Handles 10k+ products with real-time inventory.', label: 'Fullstack', icon: '🛒', tags: ['React', 'Node.js', 'PostgreSQL'], accentColor: '#3B82F6' },
  { color: '#0d0d0f', title: 'AI Chat Assistant', description: 'LLM-powered assistant with RAG pipeline, vector search, and multi-modal input support.', label: 'AI Engineer', icon: '🤖', tags: ['Python', 'LangChain', 'OpenAI'], accentColor: '#8B5CF6' },
  { color: '#0d0d0f', title: 'ML Fraud Detection', description: 'Real-time fraud detection system achieving 97.8% accuracy using ensemble models and feature engineering on financial data.', label: 'ML Engineer', icon: '🧠', tags: ['Python', 'TensorFlow', 'FastAPI'], accentColor: '#EC4899' },
  { color: '#0d0d0f', title: 'DevOps CI/CD Pipeline', description: 'Automated deployment pipeline with Docker, Kubernetes, and GitHub Actions. Reduced deploy time by 80%.', label: 'DevOps', icon: '⚙️', tags: ['Docker', 'K8s', 'GitHub Actions'], accentColor: '#10B981' },
  { color: '#0d0d0f', title: 'Network Monitor Dashboard', description: 'Real-time network topology monitoring with anomaly detection and automated alerting for enterprise infrastructure.', label: 'Network', icon: '🌐', tags: ['SNMP', 'Grafana', 'Python'], accentColor: '#F59E0B' },
  { color: '#0d0d0f', title: 'Brand Identity System', description: 'Complete visual identity with logo, design system, and component library for a SaaS startup.', label: 'UI/UX Design', icon: '🎨', tags: ['Figma', 'Illustrator', 'Design System'], accentColor: '#6366F1' },
];
    ...
    <>
      ...
      <div className="card-grid bento-section" ref={gridRef}>
        {cards.map((card, index) => {
          ...
          const cardStyle: any = { backgroundColor: card.color || '#0d0d0f', '--glow-color': glowColor };
          ...
          const content = (
            <>
              ...
              {card.accentColor && (
                <div className="magic-bento-card__decoration" style={{ background: card.accentColor }} />
              )}
              <div className="magic-bento-card__content">
                ...
                {card.tags && (
                  <div className="magic-bento-card__tags">
                    {card.tags.map((tag, i) => (
                      <span key={i} className="magic-bento-card__tag" style={{ borderColor: `${card.accentColor}30`, color: card.accentColor ? `${card.accentColor}cc` : undefined }}>{tag}</span>
                    ))}
                  </div>
                )}
              </div>
